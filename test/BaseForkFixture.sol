...
abstract contract BaseForkFixture is Test, TestConstants {
  ...
  function deployContracts() internal virtual {
    verifiedERC20Deployment = new TestVerifiedERC20Deployment({
      _hookRegistryManager: users.owner,
      _outputFilename: ''
    });
    stdstore
      .target(address(verifiedERC20Deployment))
      .sig('deployer()')
      .checked_write(users.deployer);
  ...
  function labelContracts() internal virtual {
    vm.label({
      account: address(verifiedERC20Deployment),
      newLabel: 'VerifiedERC20Deployment'
    });
    vm.label({ account: address(verifiedERC20), newLabel: 'VerifiedERC20' });
    vm.label({
      account: address(verifiedERC20Factory),
      newLabel: 'VerifiedERC20Factory'
    });
