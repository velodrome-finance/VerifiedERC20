...
contract TransferFromConcreteTest is VerifiedERC20Test {
  ...
  function test_WhenTheFromAddressIsTheZeroAddress()
    external
    whenTheAmountIsSmallerOrEqualToTheAllowance(0)
  {
    ...
    whenTheToAddressIsNotTheZeroAddress
  {
    ...
    address _to = users.bob;
    assertEq(
      verifiedERC20.allowance({ owner: _from, spender: address(this) }),
      _amount
    );
    ...
    vm.expectCall({
      ...
      data: abi.encodeCall(
        IHook.check,
        (address(this), abi.encode(_from, _to, _amount))
      ),
      count: 1
    });
    vm.expectCall({
