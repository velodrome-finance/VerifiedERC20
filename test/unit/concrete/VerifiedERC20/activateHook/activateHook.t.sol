...
contract ActivateHookConcreteTest is VerifiedERC20Test {
  ...
  function test_WhenTheHookIsNotRegisteredInTheHookRegistry()
    external
    whenTheCallerIsTheOwner
  {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IVerifiedERC20.VerifiedERC20_InvalidHook.selector,
        address(mockSuccessHook)
      )
    );
    verifiedERC20.activateHook({ _hook: address(mockSuccessHook) });
  }
    ...
    whenTheHookIsRegisteredInTheHookRegistry
  {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IVerifiedERC20.VerifiedERC20_HookAlreadyActivated.selector,
        address(mockSuccessHook)
      )
    );
    verifiedERC20.activateHook({ _hook: address(mockSuccessHook) });
    ...
    whenTheHookIsNotAlreadyActivated
  {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        IVerifiedERC20.VerifiedERC20_MaxHooksExceeded.selector
      )
    );
    verifiedERC20.activateHook({ _hook: address(mockSuccessHook) });
  }
    ...
    whenTheHookIsNotAlreadyActivated
  {
    ...
    emit IVerifiedERC20.HookActivated({
      hook: address(hook),
      entrypoint: IHookRegistry.Entrypoint.BEFORE_MINT
    });
    verifiedERC20.activateHook({ _hook: address(hook) });

    address[] memory hooksForEntrypoint = verifiedERC20.getHooksForEntrypoint(
      IHookRegistry.Entrypoint.BEFORE_MINT
    );
    assertEq(hooksForEntrypoint.length, 1);
    assertEq(hooksForEntrypoint[0], address(mockSuccessHook));

    address hookAtIndex = verifiedERC20.getHookAtIndex(
      IHookRegistry.Entrypoint.BEFORE_MINT,
      0
    );
    assertEq(hookAtIndex, address(hook));
    assertEq(
      verifiedERC20.getHooksCountForEntrypoint(
        IHookRegistry.Entrypoint.BEFORE_MINT
      ),
      1
    );
