...
contract MockSuccessHook is BaseHook {
    ...
    function supportsEntrypoint(IHookRegistry.Entrypoint _entrypoint) external pure override returns (bool) {
        return _entrypoint == IHookRegistry.Entrypoint.BEFORE_MINT || _entrypoint == IHookRegistry.Entrypoint.AFTER_MINT
            || _entrypoint == IHookRegistry.Entrypoint.BEFORE_BURN || _entrypoint == IHookRegistry.Entrypoint.AFTER_BURN
